        ...
        if (epoch + 1)%args.testAndSaveEvery == 0:
            ...
            if args.no_val == False:
                ...
                if args.training_mode == 'single':
                    ...
                    writer.add_scalar('test/1.test_acc', test_acc[0], epoch)
                    writer.add_scalar('test/1.test_f1', test_acc[1], epoch)
                    writer.add_scalar('test/1.test_hamming', test_acc[2], epoch)
                    writer.add_scalar('test/1.test_jaccard', test_acc[3], epoch)
...
def test(args, test_loader, model, epoch, training_mode='single'):
    ...
    if args.training_mode == 'single':
        ...
        for key in ground_truth_target:
            clip_values = np.array(ground_truth_target[key]).mean(axis=0)
            ##print(clip_values)
            ground_truth_target[key] = clip_values.astype(int)
        pred_values = []
        target_values = []
        for key in predicted_target:
            pred_values.append(predicted_target[key])
            target_values.append(ground_truth_target[key])
        pred_values = torch.tensor(np.array(pred_values))
